#
# ApacheConf::Config::General.pm - Generic Config Module
#
...
#
# namespace
package ApacheConf::Config::General;
...
$ApacheConf::Config::General::VERSION = "2.63";
...
sub new {
  #
  # create new ApacheConf::Config::General object
  #
  my($this, @param ) = @_;
...
sub _process {
  ...
  if (exists $self->{StringContent}) {
    ...
    if (ref($self->{ConfigHash}) eq 'HASH') {
      ...
      croak "ApacheConf::Config::General: Parameter -ConfigHash must be a hash reference!\n";
    }
  }
...
sub _blessoop {
  ...
  if ($self->{ExtendedAccess}) {
    ...
    bless $self, 'ApacheConf::Config::General::Extended';
    eval {
      require ApacheConf::Config::General::Extended;
    };
    if ($EVAL_ERROR) {
      croak "ApacheConf::Config::General: " . $EVAL_ERROR;
    }
  }
#  return $self;
}
...
sub _blessvars {
  ...
  if ($self->{InterPolateVars} || $self->{InterPolateEnv}) {
    ...
    bless $self, 'ApacheConf::Config::General::Interpolated';
    eval {
      require ApacheConf::Config::General::Interpolated;
    };
    if ($EVAL_ERROR) {
      croak "ApacheConf::Config::General: " . $EVAL_ERROR;
    }
  ...
  if ($self->{SplitPolicy} ne 'guess') {
    if ($self->{SplitPolicy} eq 'whitespace') {
      ...
      if (! $self->{SplitDelimiter} ) {
        croak "ApacheConf::Config::General: SplitPolicy set to 'custom' but no SplitDelimiter set.\n";
      }
    }
    else {
      croak "ApacheConf::Config::General: Unsupported SplitPolicy: $self->{SplitPolicy}.\n";
    }
  }
  else {
    if (!$self->{StoreDelimiter}) {
...
sub _prepare {
  ...
  if (exists $conf{-String} ) {
    ...
    # fixed bug#33385
      ...
      croak "ApacheConf::Config::General: Parameter -String must be a SCALAR or ARRAYREF!\n";
    }
    delete $conf{-String};
  }
  if (exists $conf{-Tie}) {
  ...
  foreach my $entry (keys %conf) {
    ...
    $key =~ s/^\-//;
    if (! exists $self->{$key}) {
      croak "ApacheConf::Config::General: Unknown parameter: $entry => \"$conf{$entry}\" (key: <$key>)\n";
    }
  ...
  if ($self->{UseApacheIfDefine}) {
    if (exists $conf{-Define}) {
      ...
      if ($ref eq '') {
        ...
        croak qq{ApacheConf::Config::General: Unsupported ref '$ref' for 'Define'};
      }
...
sub _open {
  ...
  if (!-e $configfile) {
    ...
    if (!$found) {
      ...
      croak qq{ApacheConf::Config::General The file "$basefile" does not exist$path_message!};
    }
  }
  ...
  if (-d $configfile and $this->{IncludeDirectories}) {
    ...
    local *INCLUDEDIR;
    opendir INCLUDEDIR, $configfile or croak "ApacheConf::Config::General: Could not open directory $configfile!($!)\n";
    ...
    croak "ApacheConf::Config::General: config file argument is a directory, expecting a file!\n";
  }
  elsif (-e _) {
...
sub _openfile_for_read {
  ...
  my $fh = IO::File->new( $file, 'r')
    or croak "ApacheConf::Config::General: Could not open $file!($!)\n";
...
sub _process_apache_ifdefine {
  ...
  if ($blockLevel) {
    croak qq(ApacheConf::Config::General: Block <IfDefine> has no EndBlock statement!\n);
  }
...
# quoted block, unquote it and do not split
        ...
        croak "ApacheConf::Config::General: EndBlock \"<\/$1>\" has no StartBlock statement (level: $this->{level}, chunk $chunk)!\n";
      }
        ...
        if (exists $config->{$option}) {
          if ($this->{MergeDuplicateOptions}) {
            ...
            if (! $this->{AllowMultiOptions} ) {
              ...
              croak "ApacheConf::Config::General: Option \"$option\" occurs more than once (level: $this->{level}, chunk $chunk)!\n";
            }
            else {
      ...
      if ($block_level) {           # this endblock is not the one we are searching for, decrement and push
        ...
        if (defined $blockname or $this->{ForceBlockName}) {
          ...
          if (ref($config->{$block}) eq '') {
            croak "ApacheConf::Config::General: Block <$block> already exists as scalar entry!\n";
          }
          elsif (ref($config->{$block}) eq 'ARRAY') {
            croak "ApacheConf::Config::General: Cannot append named block <$block $blockname> to array of scalars!\n"
            ...
            if ($this->{MergeDuplicateBlocks}) {
              ...
              if (! $this->{AllowMultiOptions}) {
                croak "ApacheConf::Config::General: Named block \"<$block $blockname>\" occurs more than once (level: $this->{level}, chunk $chunk)!\n";
              }
          ...
          if (exists $config->{$block}) {
            if (ref($config->{$block}) eq '') {
              croak "ApacheConf::Config::General: Cannot create hashref from <$block> because there is\n"
            ...
            if ($this->{MergeDuplicateBlocks}) {
              ...
              if (! $this->{AllowMultiOptions}) {
                croak "ApacheConf::Config::General: Block \"<$block>\" occurs more than once (level: $this->{level}, chunk $chunk)!\n";
              }
              else {
  ...
  if ($block) {
    ...
    croak "ApacheConf::Config::General: Block \"<$block>\" has no EndBlock statement (level: $this->{level}, chunk $chunk)!\n";
  }
  return $config;
}
...
sub save {
  ...
  if ( (@two && $one) && ( (scalar @two) % 2 == 0) ) {
    ...
    croak q(ApacheConf::Config::General: The save() method is deprecated. Use the new save_file() method instead!);
  }
  return;
}
...
sub save_file {
  ...
  if (!$file) {
    croak "ApacheConf::Config::General: Filename is required!";
  }
  else {
    if ($this->{UTF8}) {
      $fh = IO::File->new;
      open($fh, ">:utf8", $file)
        or croak "ApacheConf::Config::General: Could not open $file in UTF8 mode!($!)\n";
    }
    else {
    ...
    if (!$config) {
      if (exists $this->{config}) {
        ...
        croak "ApacheConf::Config::General: No config hash supplied which could be saved to disk!\n";
      }
    }
    else {
...
sub _store {
  ...
  foreach my $entry ( $this->{SaveSorted} ? sort keys %$config : keys %$config ) {
    # fix rt#104548
    if ($entry =~ /[<>\n\r]/) {
      croak "ApacheConf::Config::General: current key contains invalid characters: $entry!\n";
    }
...
sub _hashref {
  ...
  if ($this->{Tie}) {
    ...
    if ($EVAL_ERROR) {
      croak q(ApacheConf::Config::General: Could not create a tied hash of type: ) . $this->{Tie} . q(: ) . $EVAL_ERROR;
    }
    my %hash;
    tie %hash, $this->{Tie};
    return \%hash;
...
sub ParseConfig {
  ...
  #
  return (new ApacheConf::Config::General(@_))->getall();
}
...
sub SaveConfig {
  ...
  if (!$file || !$hash) {
    croak q{ApacheConf::Config::General::SaveConfig(): filename and hash argument required.};
  }
  else {
    if (ref($hash) ne 'HASH') {
      croak q(ApacheConf::Config::General::SaveConfig() The second parameter must be a reference to a hash!);
    }
    else {
      (new ApacheConf::Config::General(-ConfigHash => $hash))->save_file($file);
    }
  }
  return;
}
...
sub SaveConfigString {
  ...
  if (!$hash) {
    croak q{ApacheConf::Config::General::SaveConfigString(): Hash argument required.};
  }
  else {
    if (ref($hash) ne 'HASH') {
      croak q(ApacheConf::Config::General::SaveConfigString() The parameter must be a reference to a hash!);
    }
    else {
      return (new ApacheConf::Config::General(-ConfigHash => $hash))->save_string();
    }
  }
  return;
}
...
ApacheConf::Config::General - Generic Config Module
...
=head1 SYNOPSIS
 ...
 $conf = ApacheConf::Config::General->new("rcfile");
 my %config = $conf->getall;

 #
 # the procedural way
 use ApacheConf::Config::General qw(ParseConfig SaveConfig SaveConfigString);
...
The format of config files supported by B<ApacheConf::Config::General> is inspired by the well known Apache config
...
Possible ways to call B<new()>:
 ...
 $conf = ApacheConf::Config::General->new(\%somehash);

 $conf = ApacheConf::Config::General->new( %options ); # see below for description of possible options


This method returns a B<ApacheConf::Config::General> object (a hash blessed into "ApacheConf::Config::General" namespace.
...
config files which have to be included. ApacheConf::Config::General will search within this
...
The default behavior of ApacheConf::Config::General is to create an array if some junk in a
...
Well, an example will clarify things:
 ...
 my $conf = ApacheConf::Config::General->new(
         -ConfigFile => "rcfile",
         -FlagBits => {
              Mode => {
                 CLEAR    => 1,
...
Example:
 ...
 use ApacheConf::Config::General qw(ParseConfig);
 use Tie::IxHash;
...
input. See L<ApacheConf::Config::General::Interpolated> for more information.
...
access the parsed config. See L<ApacheConf::Config::General::Extended> for more information.

=item B<-StrictObjects>

By default this is turned on, which causes ApacheConf::Config::General to croak with an
...
You can influence the way how ApacheConf::Config::General decides which part of a line
...
ApacheConf::Config::General is able to notice c-style comments (see section COMMENTS).
...
into ApacheConf::Config::General to be able to parse real Apache configs.
...
This is because I<ApacheConf::Config::General> does not
...
The normal behavior of ApacheConf::Config::General is to look for whitespace in
...
If turned off, ApacheConf::Config::General will complain about multiple occurring options
...
of the line. The ApacheConf::Config::General module will concatenate those lines to one single-value.
...
the option B<-IncludeRelative> (see B<new()>) if you want that. An example:
 ...
 my $conf = ApacheConf::Config::General(
...
In this example ApacheConf::Config::General will try to include I<acl.cfg> from I</etc/crypt.d>:

 /etc/crypt.d/acl.cfg

The default behavior (if B<-IncludeRelative> is B<not> set!) will be to open just I<acl.cfg>,
wherever it is, i.e. if you did a chdir("/usr/local/etc"), then ApacheConf::Config::General will include:
...
In this example the value of $config{bgcolor} will be "#ffffcc", ApacheConf::Config::General will not treat
...
and a base directory as parameters. We tell ApacheConf::Config::General via
...
Use the module B<ApacheConf::Config::General::Extended>, which is
supplied with the ApacheConf::Config::General distribution.

=head1 VARIABLE INTERPOLATION

You can use variables inside your config files if you like. To do
that you have to use the module B<ApacheConf::Config::General::Interpolated>,
which is supplied with the ApacheConf::Config::General distribution.


=head1 EXPORTED FUNCTIONS

ApacheConf::Config::General exports some functions too, which makes it somewhat
...
Example:
 ...
 use ApacheConf::Config::General qw(SaveConfig);
 ..
...
Example:
 ...
 use ApacheConf::Config::General qw(ParseConfig SaveConfigString);
...
I recommend you to read the following documents, which are supplied with Perl:
 ...
 ApacheConf::Config::General::Extended      Object oriented interface to parsed configs
 ApacheConf::Config::General::Interpolated  Allows one to use variables inside config files
...
To debug ApacheConf::Config::General use the Perl debugger, see L<perldebug>.

=head1 DEPENDENCIES

ApacheConf::Config::General depends on the modules L<FileHandle>,
